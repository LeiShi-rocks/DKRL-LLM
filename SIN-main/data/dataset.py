...
def create_pt_geometric_dataset(units, treatment_graphs: list, outcomes=None) -> List[Data]:
    # Attempt to extract features:
    if hasattr(units, "get_covariates"):
        try:
            feats = units.get_covariates()
        except Exception as e:
            raise ValueError("Error calling get_covariates: " + str(e))
    elif isinstance(units, np.ndarray):
        # Check if the first element is a numpy.ndarray (assume that is the feature vector)
        if isinstance(units[0], np.ndarray):
            feats = np.array(units)
        # Otherwise, if elements are dictionaries, extract from "features" or "covariates"
        elif isinstance(units[0], dict):
            if "features" in units[0]:
                feats = np.array([u["features"] for u in units])
            elif "covariates" in units[0]:
                feats = np.array([u["covariates"] for u in units])
            else:
                raise ValueError("Elements in units array do not have 'features' or 'covariates' keys.")
        else:
            raise ValueError("Cannot extract features from units array; element type is {}".format(type(units[0])))
    elif isinstance(units, dict):
        if "features" in units:
            feats = units["features"]
        elif "covariates" in units:
            feats = units["covariates"]
        else:
            raise ValueError("Units dict does not have 'features' or 'covariates'.")
    else:
        raise ValueError("Unrecognized type for units.")
    
    # Convert extracted features to a tensor.
    unit_tensor = torch.FloatTensor(feats)
    data_list = []
    is_multi_relational = "edge_types" in treatment_graphs[0]
    for i in range(len(treatment_graphs)):
        c_size = treatment_graphs[i].get("c_size", 1)
        features = treatment_graphs[i].get("node_features", [])
        edge_index = treatment_graphs[i].get("edges", [])
        one_hot_encoding = treatment_graphs[i].get("one_hot_encoding", [1.0])
